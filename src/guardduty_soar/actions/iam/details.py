...
class GetIamPrincipalDetailsAction(BaseAction):
    ...
    def _get_user_details(self, user_name: str) -> Dict[str, Any]:
        ...
        attached_policies = self.iam_client.list_attached_user_policies(UserName=user_name).get("AttachedPolicies", [])
        inline_policy_names = self.iam_client.list_user_policies(UserName=user_name).get("PolicyNames", [])
    ...
    def _get_role_details(self, role_name: str) -> Dict[str, Any]:
        ...
        inline_policy_names = self.iam_client.list_role_policies(RoleName=role_name).get("PolicyNames", [])
    ...
    def execute(self, event: GuardDutyEvent, **kwargs) -> ActionResponse:
        ...
        if not principal_details:
            return {"status": "error", "details": "Required 'principal_details' were not provided."}
